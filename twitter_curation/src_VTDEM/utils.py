...
from datasets import load_dataset, DatasetDict, disable_caching, disable_progress_bar
...
class CustomTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        loss_fct = nn.CrossEntropyLoss(
            weight=torch.tensor([1.0, 1.0, 1.0], device=next(model.parameters()).device)
        )
        loss = loss_fct(
...
class CustomDinoTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False):
        labels = inputs.pop("labels")
